// g++ -std=c++11 -o realsense_pi realsense_pi.cpp -lrealsense2 `pkg-config --cflags --libs opencv4`
...
try
{
    ...
    while (true)
    {
        ...
        cv::flip(depth_image, depth_image, 0); // 0 specifies a flip around the x-axis (vertical flip)
        cv::flip(color_image, color_image, 0); // 0 specifies a flip around the x-axis (vertical flip)
